...
void stop_mode()
{
    ...
    ROS_INFO("dist_stop %f" , dist_stop);
    if (dist_stop<=0.21 || robot_in_tolerance)
    {
        path_stored=false;
...
void correction_final_theta()
{
 ...
 //   ROS_INFO("Final theta corrected: %d", final_theta_corrected);
    ...
    if (abs(err.theta)<0.06 && !final_theta_corrected && robot_in_tolerance)
    {
        robot_reached=true;
        v_cmd = 0;
        w_cmd = 0;
...
int main(int argc, char **argv) 
{
    ...
    while (ros::ok()) 
    {
        ...
        if((timestamp_now - time_grid_map)>2 && path_theta_published)
        {
        ...
        if((timestamp_now - time_local_map)>2 && path_theta_published)
        {
            ROS_ERROR("Local Map Lidar is Failed 2");
            state_maps = false;
        }
        else
        {
